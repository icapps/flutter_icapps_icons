class CaseUtil {
  ...
  String _getPascalCase({String separator = ''}) => _words.map(_upperCaseFirstLetter).toList().join(separator);

  String _getSnakeCase({String separator = '_'}) => _words.map((word) => word.toLowerCase()).toList().join(separator);

  String _upperCaseFirstLetter(String word) => '${word.substring(0, 1).toUpperCase()}${word.substring(1).toLowerCase()}';
}
